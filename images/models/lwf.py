...
class Lwf(ContinualModel):
    ...
    def begin_task(self, dataset):
        ...
        if self.current_task > 0:
            # warm-up
            opt = SGD(self.net.classifier1.parameters(), lr=self.args.lr)
            for epoch in range(self.args.n_epochs):
                for i, data in enumerate(dataset.train_loader):
                    ...
                    outputs = self.net.classifier1(feats)[:, mask]
    ...
    def observe(self, inputs, labels, not_aug_inputs, logits=None, epoch=0):
        self.opt.zero_grad()
        outputs = self.net(inputs)['logits1']
