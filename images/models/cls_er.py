...
class CLSER(ContinualModel):
    ...
    def observe(self, inputs, labels, not_aug_inputs, task_ids=None, epoch=0):
        ...
        if not self.buffer.is_empty():
            ...
            stable_model_logits = self.ema_model(buf_inputs)['logits1']
            plastic_model_logits = self.plastic_model(buf_inputs)['logits1']
            ...
            l_cons = torch.mean(self.consistency_loss(self.net(buf_inputs)['logits1'], ema_logits.detach()))
        ...
        ce_loss = self.loss(outputs['logits1'], labels)
        loss += ce_loss
