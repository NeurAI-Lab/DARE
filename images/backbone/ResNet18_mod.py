...
class ResNetM(nn.Module):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
                 num_classes: int, nf: int, num_rot=0) -> None:
        """
        ...
        if num_rot > 0:
            self.classifier3 = nn.Sequential(
                ...
                # nn.BatchNorm1d(nf * 8 * block.expansion),
                # nn.ReLU(inplace=True),
                nn.Linear(nf * 8 * block.expansion, 4)
            )
        else:
    ...
    def freeze(self, name):
        ...
        if name == 'backbone':
            ...
            freeze_parameters(self.classifier3, requires_grad=requires_grad)
    ...
    def unfreeze(self):
        ...
        freeze_parameters(self.classifier3, requires_grad=False)
        self.train()
...
def resnet18_mod(nclasses: int, nf: int=64, num_rot=0):
    """
    ...
    """
    return ResNetM(BasicBlock, [2, 2, 2, 2], nclasses, nf, num_rot=num_rot)
    # resnet = models.resnet18(pretrained=False)
    # resnet.fc = torch.nn.Linear(resnet.fc.in_features, nclasses)
    # return resnet


def get_resnet18_mod(nclasses: int=1000, nf: int=64):
    """
    Instantiates a ResNet18 network.
    :param nclasses: number of output classes
    :param nf: number of filters
    :return: ResNet network
    """
    return ResNetM(BasicBlock, [2, 2, 2, 2], nclasses, nf)