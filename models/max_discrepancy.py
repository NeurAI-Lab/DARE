...
class MaxDiscrepancy(ContinualModel):
    NAME = 'max_discrepancy'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(MaxDiscrepancy, self).__init__(backbone, loss, args, transform)
        ...
        self.ema_classifier = self.ema_update_freq > 0
    ...
    def stepd(self):
        # Step D: Learn Classifier 1 only on buffered samples
        self.opt.zero_grad()
        if self.opt2 is not None:
            self.opt2.zero_grad()
        loss = 0
        # CE for buffered samples
        loss_aux_ce, loss_aux_logit = torch.tensor(0, device=self.device), torch.tensor(0, device=self.device)
        if not self.buffer.is_empty():
            buf_inputs, buf_labels, buf_logits1, buf_logits2, _ = self.buffer.get_data(
        ...
        return loss.item(), loss_aux_ce.item(), loss_aux_logit.item()
    ...
    def update_ema_model_variables(self):
        ...
        for ema_param, param in zip(self.net.classifier3.parameters(), self.net.classifier1.parameters()):
    ...
    def end_task(self, dataset):
        ...
        if self.args.linear_alpha > 0 and self.ema_update_freq==0:
            c2 = self.net.classifier2.state_dict()
            sd = self.net.classifier1.state_dict()
            for name, param in self.net.classifier1.named_parameters():
                sd[name] = self.args.linear_alpha * param.data + \
                           (1 - self.args.linear_alpha) * c2[name].data
            self.net.classifier1.load_state_dict(sd)
